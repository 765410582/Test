...
export class ChessBoardMgr extends Component {
    ...
    initView() {
        this.reBtn = this.node.getChildByName("reBtn");
        this.reBtn.on("click", this.onReturn, this);
    }
    ...
    handleTouchEvent(eventTouch, eventType) {
        ...
        console.log("handleTouchEvent",eventType);
        let spacePos = ToolHelper.screenPosToSpacePos(eventTouch, this.node);
        ...
        if (pos) {
            ...
            new Promise((resolve, reject) => {
                ...
                this.gridView.getSelectNode().setPosition(view.getPosition())
                let changeModels = this.selectSell(pos)
                if (eventType === Node.EventType.TOUCH_START) {
                    this.isCanMove = changeModels.length < 3;
                    console.log("this.isCanMove",this.isCanMove);
                }
            }).catch(err => {
